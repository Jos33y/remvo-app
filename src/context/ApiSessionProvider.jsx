 ...
 * from a PSP webhook, not a button; the amount is set by the
...
/* The display name on the Kora dynamic VBA. We pass 'Remvo' to Kora
 * at charge creation (modules/kora/client.js, createBankTransferCharge:
 * accountName) and Kora honours it on the resolved account. The
 * normalize() fallback below preserves UX even if a future GET
 * response stops echoing this field. */
...
export function ApiSessionProvider({ token, children }) {
  ...
  const value = useMemo(
    () => ({
      session,
      // Confirmation is PSP-driven | nothing to simulate.
